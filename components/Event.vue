<template>
  <div>
    <a
      ...
      :class="{ 'border-gray-800': event.slug, 'border-gray-200': !event.slug }"
    >
    ...
    <div
      v-if="event.save_the_date"
      class="border-2 border-gray-200 w-full flex flex-col"
    >
      <div class="relative">
        <img :src="`/img/events/og/${event.logo}`" alt="" />
        <span
      ...
      <div class="p-4 flex-1 flex flex-col justify-between bg-gray-100">
        <h2 class="font-bold mb-2" :class="{ 'text-2xl': !slim }">
          {{ event.title }}
        </h2>
        <div class="text-xs">
